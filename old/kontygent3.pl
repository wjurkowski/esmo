...
#		print "dupa\n";
	...
	if(params{'limit_row'} eq 'SAletters'){
		...
		my @CT=();
		 ...
		 open(TOUT,">$pre_SA_vs_hydropbocity-CT.dat") or die "I can’t open and write output file: $!\n";
		 }
		}
	}

 foreach $kat(@aatab){
 chdir "$rootdir/$library/$libseq/$kat/" or die "Nie moge wejsc do katalogu $kat $!\n";
...
#			}
			...
			$j++;
			$CT[$wart][$hfobc][$j]=$CT[$wart][$hfobc][$j]+$para[2];
			}
